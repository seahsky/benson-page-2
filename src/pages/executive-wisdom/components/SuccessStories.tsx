...
export default function SuccessStories({ content, language }: SuccessStoriesProps) {
  ...
  const getIndustryIcon = (industry?: string) => {
    ...
    if (industry.includes('Government') || industry.includes('政府')) return Award
    if (industry.includes('Management') || industry.includes('管理')) return TrendingUp
    return Users
  }
      ...
      <div className="container-custom">
        ...
        <div className="bg-gradient-to-r from-primary to-primary/80 rounded-2xl p-8 md:p-12 text-white mb-12 fade-in-up stagger-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            <div>
              ...
              <p className={`text-lg ${language === 'zh' ? 'font-chinese' : ''}`}>
                {language === 'zh' ? '成功案例' : 'Success Stories'}
              </p>
            </div>
            <div>
              <div className="text-4xl md:text-5xl font-bold mb-2 text-secondary">95%</div>
              <p className={`text-lg ${language === 'zh' ? 'font-chinese' : ''}`}>
          ...
          {[
            {
              ...
              description: language === 'zh' ? '成功轉換到理想職位' : 'Successfully transitioned to dream positions'
            },
            {
              icon: Award,
              title: language === 'zh' ? '政府職位' : 'Government Roles',
              description: language === 'zh' ? '獲得公務員永久職位' : 'Secured permanent public service positions'
            },
            {
              icon: Users,
              title: language === 'zh' ? '自信建立' : 'Confidence Building',
              description: language === 'zh' ? '重建職場自信心' : 'Rebuilt workplace confidence'
            },
            {
              icon: Quote,
              title: language === 'zh' ? '技能轉換' : 'Skill Transfer',
              description: language === 'zh' ? '成功包裝轉移技能' : 'Successfully packaged transferable skills'
            }
          ].map((outcome, index) => (
            <div key={index} className="text-center p-6 bg-white rounded-lg shadow-sm border border-neutral-100 hover:shadow-md transition-all duration-300">
        ...
        <div className="text-center fade-in-up stagger-6">
          <h3 className={`text-2xl md:text-3xl font-bold text-neutral-800 mb-4 ${
            ...
            {language === 'zh' ? '準備好創造您的成功故事了嗎？' : 'Ready to Create Your Success Story?'}
          </h3>
